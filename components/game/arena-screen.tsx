...
function CardView({ row, hidden = false, selected = false, onClick, onEffect, onInspect, ownerPreview = false }: { row: VisibleMatchCard; hidden?: boolean; selected?: boolean; onClick?: () => void; onEffect?: () => void; onInspect?: (card:VisibleMatchCard)=>void; ownerPreview?: boolean }) {
  return <motion.div layout className={`relative h-36 w-24 shrink-0 overflow-visible rounded-xl transition-all ${selected ? "z-[60] cursor-crosshair ring-4 ring-blue-400 shadow-[0_0_28px_rgba(96,165,250,.9)]" : ""}`}>
...
function Inspector({ card, mana, used, canAct, onActivate }: { card: VisibleMatchCard | null; mana:number; used:boolean; canAct:boolean; onActivate:(card:VisibleMatchCard)=>void }) {
  ...
  const manual=card.card_data.effect_definition?.find(effect=>effect.trigger_type==="manual"||effect.trigger_type==="reaction"); const cost=Number(manual?.parameters?.mana_cost??card.card_data.mana??0); const disabled=!manual||!canAct||used||mana<cost
...
function actionText(action: MatchAction, state: MatchState | null, cards:VisibleMatchCard[] = []) {
  ...
  if(action.action_type==="card_played"&&p.destination_zone==="attacker")return `⚔️ ${actor??"Jogador"} invocou ${source?.card_data?.nome??"uma carta"} (${source?.current_power??"?"} ATK / ${source?.current_life??"?"} HP) para o Campo de Ataque.`
  ...
  if(action.action_type==="attack_declared"){const names=(Array.isArray(p.attacker_card_ids)?p.attacker_card_ids:[]).map(id=>card(id)?.card_data?.nome??"Carta").join(" + ");return `🔥 ${actor??"Jogador"} declarou ataque com ${names||"suas unidades"}, totalizando ${String(p.total_power??p.declared_power??"?")} de Poder contra as defesas inimigas.`}
  if(action.action_type==="attack_resolved"){const reinforcements=Array.isArray(p.reinforcements)?p.reinforcements as Array<Record<string,unknown>>:[];const destroyed=reinforcements.filter(item=>(item.result as Record<string,unknown>|undefined)?.destroyed).length;return `💥 O ataque de ${String(p.total_power??"?")} de dano atingiu os reforços em ordem; ${destroyed} carta(s) foram destruídas e todo excedente após uma única Carta de Vida foi anulado.`}
  if(action.action_type==="effect_activated")return `✨ ${actor??"Jogador"} gastou ${String(p.mana_spent??0)} de Mana e conjurou o efeito de ${source?.card_data?.nome??"uma carta"}: ${source?.card_data?.efeito??String(p.effect_code??"efeito resolvido")}.`
  ...
  if(action.action_type==="turn_ended"){const nextName=p.active_player_id===state?.player1_id?state?.player1_username:state?.player2_username;return Number(p.new_turn)===2?`Turno ${turn}: ${actor??"Jogador"} encerrou o turno. ${nextName??"O segundo jogador"} inicia sua primeira jogada sem compra adicional.`:`Turno ${turn}: ${actor??"Jogador"} encerrou o turno. 🃏 ${nextName??"O jogador ativo"} comprou 1 carta do grimório no início do novo turno.`}
...
export function ArenaScreen() {
  ...
  const [effectBanner,setEffectBanner]=useState<{player:string;card:string;description:string}|null>(null)
  const botRunning = useRef(false)
  const setupTimer=useRef<number|null>(null)
  const seenEffect=useRef<number|null>(null)
  ...
  useEffect(()=>()=>{if(setupTimer.current!==null)window.clearTimeout(setupTimer.current)},[])
  const duel = useDuelRealtime(matchId, userId)
  const { matchState, boardCards, matchActions, pendingAttack, pendingEffectChoice, connectionStatus, isTraining, isCurrentPlayer, isPlayer1, opponentId, hasActedThisTurn, reactionUsed,usedEffectCardIds,isActionPending } = duel
  ...
  useEffect(() => {
    ...
    if(seenAction.current===null){seenAction.current=max;setVisibleLogSequence(max);return}
  ...
  useEffect(()=>{
    ...
    const finish=window.setTimeout(()=>{setScreenShake(false);setVisibleLogSequence(previous=>Math.max(previous,showcase.action.sequence_number));setShowcase(null);setVisualCards(boardCards)},1800)
  ...
  useEffect(()=>{const action=[...matchActions].reverse().find(item=>item.action_type==="effect_activated");if(!action||seenEffect.current===action.id)return;seenEffect.current=action.id;const cardId=String(action.payload_public.source_card_id??"");const card=boardCards.find(item=>item.id===cardId);const player=action.actor_user_id===matchState?.player1_id?matchState.player1_username:matchState?.player2_username;setEffectBanner({player:player??"Jogador",card:card?.card_data?.nome??"Carta",description:card?.card_data?.efeito??String(action.payload_public.effect_code??"Efeito resolvido")});const timer=window.setTimeout(()=>setEffectBanner(null),2000);return()=>window.clearTimeout(timer)},[boardCards,matchActions,matchState?.player1_id,matchState?.player1_username,matchState?.player2_username])
  ...
  const reactions = boardCards.filter(card => card.controller_user_id === userId && ["hand", "life", "reinforcement"].includes(card.zone) && (card.zone === "hand" || (card.current_life ?? 0) > 0) && card.card_data?.effect_definition?.some(effect => effect.trigger_type === "reaction" || effect.is_reaction))
  ...
  const firstImpacted=orderedDefenses[0]?.id
  const validReactions=reactions.filter(card=>{const effect=card.card_data?.effect_definition?.find(item=>item.trigger_type==="reaction"||item.is_reaction);if(!effect)return false;const cost=Number(effect.parameters?.mana_cost??card.card_data?.mana??0);if(cost>myMana)return false;if(card.zone==="hand")return effect.parameters?.reaction_from_hand===true||(Array.isArray(effect.parameters?.allowed_zones)&&effect.parameters.allowed_zones.includes("hand"));return card.zone==="life"||card.id===firstImpacted})
  ...
  return <motion.main animate={screenShake?{x:[0,-8,7,-5,3,0],y:[0,4,-3,2,0]}:{x:0,y:0}} transition={{duration:.35}} className={`relative h-screen overflow-hidden bg-[url('/yang-69TcSUVhbmY-unsplash.jpg')] bg-cover bg-center bg-no-repeat text-stone-100 ${effectSelection?"cursor-crosshair":""}`}><div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/60 to-black/85 backdrop-blur-[2px]" />
    ...
    <AnimatePresence>{effectBanner&&<motion.div initial={{opacity:0,scale:.8,y:-30}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:1.08}} className="pointer-events-none fixed inset-x-0 top-1/3 z-[188] mx-auto w-[min(900px,90vw)] border-y-2 border-purple-300 bg-gradient-to-r from-transparent via-purple-950/95 to-transparent px-10 py-6 text-center shadow-[0_0_50px_rgba(192,132,252,.55)]"><Sparkles className="mx-auto mb-2 text-purple-200"/><p className="font-serif text-2xl font-black uppercase text-purple-50">✨ {effectBanner.player} ativou o efeito de {effectBanner.card}</p><p className="mt-2 text-sm text-purple-200">{effectBanner.description}</p></motion.div>}</AnimatePresence>
    <div className="relative z-10 grid h-[calc(100vh-132px)] grid-cols-[190px_minmax(580px,1fr)_170px] gap-2 overflow-x-auto p-2">
      ...
      <section className="flex min-w-[580px] min-h-0 flex-col justify-between overflow-y-auto overflow-x-visible rounded-xl border border-amber-700/20 bg-black/20 px-2">
        ...
        <div className="z-40 my-1 flex items-center justify-between gap-3 border-y border-amber-700/50 bg-stone-950/95 px-4 py-2 shadow-2xl">
          ...
          <div className="flex items-center gap-2">{isCurrentPlayer ? (matchState?.my_actions_this_turn??0)>0 || attackers.length ? <button onClick={() => void commitTurn()} className="rounded border border-amber-400 bg-amber-700 px-4 py-2 text-[10px] font-black text-amber-100 shadow-[0_0_15px_rgba(245,158,11,.3)] hover:bg-amber-600">{attackers.length ? `ENCERRAR E ATACAR (${attackers.length})` : "ENCERRAR TURNO"}</button> : <button onClick={() => void duel.passWithoutAction()} className="rounded border border-blue-400 bg-blue-900 px-4 py-2 text-[10px] font-black text-blue-100">PASSAR E COMPRAR</button> : <span className="rounded border border-stone-700 bg-black/60 px-4 py-2 text-[10px] font-black text-stone-500">AGUARDE O OPONENTE</span>}<button disabled={!isCurrentPlayer} onClick={() => void duel.surrenderMatch()} className="rounded border border-red-800 bg-red-950 px-2 py-2 text-red-300 disabled:opacity-30" title="Se render"><Flag size={14} /></button></div>
    ...
    <AnimatePresence>{matchState?.status === "ban_phase" && <motion.div initial={{opacity:0}} animate={{opacity:1}} className="fixed inset-0 z-[170] flex items-center justify-center bg-black/95 p-5"><div className="max-h-[94vh] w-full max-w-7xl overflow-y-auto rounded-2xl border border-amber-400 bg-stone-950 p-6 shadow-[0_0_70px_rgba(245,158,11,.2)]"><h2 className="text-center font-serif text-3xl font-black text-amber-100">Banimento Estratégico Ofieri</h2><p className="mb-5 mt-1 text-center text-sm text-stone-400">Clique em uma carta para travar a inspeção e decidir qual estratégia remover.</p>{banError&&<div className="mb-4 rounded border border-red-500 bg-red-950/80 p-3 text-sm text-red-100">{banError}</div>}<div className="grid gap-6 lg:grid-cols-[1fr_330px]"><div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-5">{banCandidates.map(card=><button disabled={banBusy} key={card.card_id} onClick={()=>setSelectedBan(card)} className={`overflow-hidden rounded-xl border-2 bg-black p-1 transition hover:-translate-y-2 ${selectedBan?.card_id===card.card_id?"border-red-400 shadow-[0_0_28px_rgba(248,113,113,.5)]":"border-amber-700/50"}`}><img src={card.image_url} alt={card.name} className="aspect-[2/3] w-full object-contain"/><span className="block p-2 text-xs font-black text-amber-100">{card.name}<small className="block uppercase text-stone-500">{card.rarity} · {card.copy_count} cópia(s)</small></span></button>)}</div><aside className="sticky top-0 rounded-xl border border-red-500/40 bg-black/70 p-4">{selectedBan?<><div className="mx-auto aspect-[2/3] w-full max-w-[290px] overflow-hidden rounded-xl border-4 border-red-400 bg-black shadow-[0_0_35px_rgba(239,68,68,.45)]"><img src={selectedBan.image_url} alt={selectedBan.name} className="h-full w-full object-contain"/></div><h3 className="mt-3 text-center font-serif text-xl font-black text-amber-100">{selectedBan.name}</h3><p className="text-center text-xs uppercase text-red-300">{selectedBan.rarity} · maior raridade detectada</p><button disabled={banBusy} onClick={()=>void submitBan(selectedBan.card_id)} className="mt-4 w-full rounded-lg border-2 border-red-300 bg-gradient-to-r from-red-950 via-red-700 to-red-950 px-4 py-4 font-black text-red-50 disabled:opacity-40">{banBusy?"VALIDANDO BANIMENTO…":"🚫 BANIR ESTA CARTA DA PARTIDA"}</button></>:<p className="py-20 text-center text-stone-500">Nenhuma candidata foi retornada pelo servidor.</p>}</aside></div></div></motion.div>}</AnimatePresence>

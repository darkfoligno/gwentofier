  ...
  return <main className="relative h-screen overflow-hidden bg-[url('/yang-69TcSUVhbmY-unsplash.jpg')] bg-cover bg-center bg-no-repeat text-stone-100"><div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/60 to-black/85 backdrop-blur-[2px]" />
    <div className="relative z-10 grid h-[calc(100vh-150px)] grid-cols-[220px_minmax(620px,1fr)_190px] gap-3 overflow-x-auto p-3">
      ...
      <section className="min-w-[620px] overflow-y-auto rounded-xl border border-amber-700/20 bg-black/20 px-2">
        <Zone label="Campo de Defesa do oponente" cards={theirs("reinforcement")} hidden /><Zone label="Cartas de Vida do oponente" cards={theirs("life")} slots={3} danger={Boolean(pendingAttack)} /><Zone label="Campo de Ataque do oponente" cards={theirs("attacker")} />
        ...
        <Zone label="Campo de Ataque" cards={attackers} selected={selectedAttackers} onCard={toggleAttacker} onEmpty={position => playSelected("attacker", position)} /><Zone label="Campo de Defesa" cards={mine("reinforcement")} hidden ownerPreview onEmpty={position => playSelected("reinforcement", position)} /><Zone label="Suas cartas de vida" cards={mine("life")} slots={3} danger onEmpty={selectedCard && matchState && matchState.current_turn > 0 && matchState.current_turn < 4 ? position => void duel.replaceEarlyLifeCard(selectedCard.id, position).then(() => setSelectedHand(null)) : undefined} />

...
function CardView({ row, hidden = false, selected = false, onClick, onEffect, ownerPreview = false }: { row: VisibleMatchCard; hidden?: boolean; selected?: boolean; onClick?: () => void; onEffect?: () => void; ownerPreview?: boolean }) {
  return <motion.div layout className={`relative aspect-[2/3] w-[clamp(54px,6.5vw,92px)] shrink-0 rounded-xl transition-all ${selected ? "ring-2 ring-red-400 shadow-[0_0_24px_rgba(248,113,113,.8)]" : ""}`}>
    <button type="button" onClick={onClick} className="h-full w-full"><GameCard card={row.card_data ?? undefined} isFaceDown={hidden} interactive={Boolean(onClick)} /></button>
    {!hidden && <motion.span key={`${row.id}-${row.current_life}`} initial={{ scale: 1.5, backgroundColor: "#dc2626" }} animate={{ scale: 1, backgroundColor: "#450a0a" }} className="pointer-events-none absolute -bottom-1 -right-1 z-40 rounded-full border border-red-300 px-1.5 py-0.5 text-[8px] font-black text-white">♥ {row.current_life ?? 0}/{row.maximum_life ?? 0}</motion.span>}
    ...
    {onEffect && !hidden && <button type="button" onClick={event => { event.stopPropagation(); onEffect() }} className="absolute -right-2 -top-2 z-50 flex h-6 w-6 items-center justify-center rounded-full border border-cyan-300 bg-blue-900 text-cyan-100" title="Ativar efeito">✦</button>}
  </motion.div>
}

function Zone({ label, cards, slots = 4, hidden = false, selected, onCard, onEffect, onEmpty, onDropCard, danger = false, ownerPreview = false }: { label: string; cards: VisibleMatchCard[]; slots?: number; hidden?: boolean; selected?: Set<string>; onCard?: (card: VisibleMatchCard) => void; onEffect?: (card: VisibleMatchCard) => void; onEmpty?: (position: number) => void; onDropCard?: (cardId: string, position: number) => void; danger?: boolean; ownerPreview?: boolean }) {
  const occupied = new Set(cards.map(card => card.slot_index))
  return <section className="min-h-0 min-w-0"><div className="flex items-center gap-2"><span className="h-px flex-1 bg-gradient-to-r from-transparent to-amber-600/30" /><h3 className="font-serif text-[8px] font-black uppercase tracking-[.18em] text-amber-200/70">{label}</h3><span className="h-px flex-1 bg-gradient-to-l from-transparent to-amber-600/30" /></div><div className="flex h-[clamp(78px,11vh,138px)] items-center justify-center gap-2 overflow-hidden px-2 py-1">
    {cards.map(card => <CardView key={card.id} row={card} hidden={hidden && !card.is_face_up} selected={selected?.has(card.id)} onClick={onCard ? () => onCard(card) : undefined} onEffect={onEffect && card.card_data?.effect_definition?.some(effect => ["manual","reaction"].includes(effect.trigger_type ?? "")) ? () => onEffect(card) : undefined} ownerPreview={ownerPreview} />)}
...
export function ArenaScreen() {
  ...
  return <main className="relative h-screen overflow-hidden bg-[url('/yang-69TcSUVhbmY-unsplash.jpg')] bg-cover bg-center bg-no-repeat text-stone-100"><div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/60 to-black/85 backdrop-blur-[2px]" />
    ...
    <AnimatePresence>{matchState?.status === "ban_phase" && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[170] flex items-center justify-center bg-black/90 p-6"><div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-xl border border-amber-500 bg-stone-950 p-6"><h2 className="mb-1 text-center font-serif text-2xl font-black text-amber-200">Banimento Ofieri</h2><p className="mb-6 text-center text-sm text-stone-400">Escolha uma carta entre as de maior raridade encontradas no deck adversário. Todas as cópias serão banidas desta batalha.</p><div className="grid grid-cols-2 gap-5 sm:grid-cols-4 lg:grid-cols-6">{banCandidates.map(card => <button key={card.card_id} onClick={() => void duel.submitBan(card.card_id)} className="overflow-hidden rounded-lg border border-amber-500/50 bg-black transition hover:-translate-y-1 hover:border-red-400"><img src={card.image_url} alt={card.name} className="aspect-[2/3] w-full object-contain" /><span className="block p-2 font-serif text-xs font-bold text-amber-200">{card.name}<small className="block text-stone-500">{card.rarity} · {card.copy_count}x</small></span></button>)}</div></div></motion.div>}</AnimatePresence>

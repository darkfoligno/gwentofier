...
function CardView({ row, hidden = false, selected = false, onClick, onEffect, ownerPreview = false }: { row: VisibleMatchCard; hidden?: boolean; selected?: boolean; onClick?: () => void; onEffect?: () => void; ownerPreview?: boolean }) {
  return <motion.div layout className={`relative aspect-[2/3] w-[72px] shrink-0 rounded-xl transition-all lg:w-[86px] ${selected ? "ring-2 ring-red-400 shadow-[0_0_24px_rgba(248,113,113,.8)]" : ""}`}>
...
function Zone({ label, cards, slots = 4, hidden = false, selected, onCard, onEffect, onEmpty, danger = false, ownerPreview = false }: { label: string; cards: VisibleMatchCard[]; slots?: number; hidden?: boolean; selected?: Set<string>; onCard?: (card: VisibleMatchCard) => void; onEffect?: (card: VisibleMatchCard) => void; onEmpty?: (position: number) => void; danger?: boolean; ownerPreview?: boolean }) {
...
export function ArenaScreen() {
  ...
  return <main className="relative h-screen overflow-hidden bg-[url('/yang-69TcSUVhbmY-unsplash.jpg')] bg-cover bg-center bg-no-repeat text-stone-100"><div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/60 to-black/85 backdrop-blur-[2px]" />
    <div className="relative z-10 grid h-[calc(100vh-150px)] grid-cols-[220px_minmax(620px,1fr)_190px] gap-3 overflow-x-auto p-3">
      ...
      <section className="min-w-[620px] overflow-y-auto rounded-xl border border-amber-700/20 bg-black/20 px-2">
        ...
        <div className="sticky top-0 z-40 my-2 flex items-center justify-between gap-3 border-y border-amber-700/50 bg-stone-950/95 px-4 py-2 shadow-2xl">
          ...
          <div className="flex items-center gap-2"><button disabled={!isCurrentPlayer} onClick={() => void duel.endTurn()} className="rounded border border-amber-400 bg-amber-700 px-4 py-2 text-[10px] font-black text-amber-100 shadow-[0_0_15px_rgba(245,158,11,.3)] hover:bg-amber-600 disabled:opacity-40">ENCERRAR TURNO</button><button disabled={!isCurrentPlayer || hasActedThisTurn} onClick={() => void duel.passWithoutAction()} className="rounded border border-blue-400 bg-blue-900 px-4 py-2 text-[10px] font-black text-blue-100 disabled:opacity-40">PASSAR TURNO</button><button onClick={() => void duel.surrenderMatch()} className="rounded border border-red-800 bg-red-950 px-2 py-2 text-red-300" title="Se render"><Flag size={14} /></button></div>
    ...
    <div className="absolute bottom-0 left-1/2 z-50 flex h-[150px] w-[min(900px,90vw)] -translate-x-1/2 items-end justify-center overflow-x-auto rounded-t-[30px] border-x border-t border-amber-600/30 bg-black/55 px-12 pb-2 pt-6 backdrop-blur-md"><div className="flex justify-center -space-x-8 transition-all duration-300 hover:space-x-2">{hand.map(card => <div key={card.id} className={`relative aspect-[2/3] w-24 shrink-0 origin-bottom transition-transform duration-200 hover:z-50 hover:-translate-y-10 hover:scale-110 ${selectedHand === card.id ? "z-40 -translate-y-6 rounded-xl ring-2 ring-amber-300" : ""}`}><button onClick={() => { if (!chooseEffectTarget(card)) setSelectedHand(card.id) }} className="h-full w-full"><GameCard card={card.card_data ?? undefined} interactive /></button>{card.card_data?.effect_definition?.some(effect => effect.trigger_type === "manual") && <button onClick={() => activateEffect(card)} className="absolute -right-1 -top-1 z-50 h-7 w-7 rounded-full border border-cyan-300 bg-blue-900 text-cyan-100" title="Ativar efeito">✦</button>}</div>)}</div></div>

...
function Inspector({ card, mana, used, canAct, onActivate }: { card: VisibleMatchCard | null; mana:number; used:boolean; canAct:boolean; onActivate:(card:VisibleMatchCard)=>void }) {
  ...
  return <div className="flex h-full min-h-0 flex-col"><h2 className="mb-2 flex items-center gap-2 font-serif text-sm font-black uppercase text-amber-200"><BookOpen size={16}/> Grimório</h2><div className="mx-auto aspect-[2/3] w-full max-w-[210px] min-h-0"><GameCard card={{...card.card_data,ataque:card.current_power ?? card.card_data.ataque,vida:card.current_life ?? card.card_data.vida}} enableZoom={false}/></div>{manual&&<button disabled={disabled} onClick={()=>onActivate(card)} className="mt-2 animate-pulse rounded-lg border-2 border-yellow-200 bg-gradient-to-r from-amber-900 via-yellow-700 to-amber-900 px-2 py-2 text-[9px] font-black text-yellow-50 shadow-[0_0_18px_rgba(250,204,21,.45)] disabled:animate-none disabled:border-stone-700 disabled:bg-stone-900 disabled:text-stone-500 disabled:shadow-none">{used?"EFEITO JÁ UTILIZADO NO TURNO":mana<cost?`MANA INSUFICIENTE · CUSTO ${cost}`:`✨ ATIVAR EFEITO · CUSTO ${cost} MANA`}</button>}<section className="mt-3 min-h-0 overflow-y-auto rounded-lg border border-purple-700/30 bg-black/45 p-2"><h3 className="mb-2 text-[9px] font-black uppercase tracking-wider text-purple-200">Modificadores ativos</h3><div className="flex flex-wrap gap-1">{card.active_modifiers?.map(mod => <span key={mod.id} title={JSON.stringify(mod.metadata)} className={`rounded-full border px-2 py-1 text-[8px] font-black ${mod.modifier_type==="buff"?"border-emerald-400 bg-emerald-950 text-emerald-200":mod.modifier_type==="debuff"||mod.modifier_type==="damage"?"border-red-400 bg-red-950 text-red-200":"border-purple-400 bg-purple-950 text-purple-200"}`}>{labels[mod.modifier_type]??mod.modifier_type.toUpperCase()}{mod.power_delta?` ${mod.power_delta>0?"+":""}${mod.power_delta} ATK`:""}{mod.max_life_delta?` ${mod.max_life_delta>0?"+":""}${mod.max_life_delta} VIDA`:""}</span>)}{!card.active_modifiers?.length && <span className="text-[9px] text-stone-600">Nenhum modificador ativo.</span>}</div></section></div>
}
...
export function ArenaScreen() {
  ...
  const confirmPreparation=()=>{if(setupCards.size!==3||isActionPending)return;if(!isTraining){void submitPreparation();return}setSetupBotWaiting(true);const delay=45_000+Math.floor(Math.random()*15_001);setupTimer.current=window.setTimeout(()=>void submitPreparation(),delay)}
  ...
  return <motion.main animate={screenShake?{x:[0,-8,7,-5,3,0],y:[0,4,-3,2,0]}:{x:0,y:0}} transition={{duration:.35}} className={`relative h-screen overflow-hidden bg-[url('/yang-69TcSUVhbmY-unsplash.jpg')] bg-cover bg-center bg-no-repeat text-stone-100 ${effectSelection?"cursor-crosshair":""}`}><div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/60 to-black/85 backdrop-blur-[2px]" />
    ...
    <AnimatePresence>{selectedCard&&isCurrentPlayer&&!effectSelection&&<motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:20}} className="fixed bottom-36 left-1/2 z-[90] flex -translate-x-1/2 gap-3 rounded-xl border border-amber-300/50 bg-black/90 p-3 shadow-[0_0_28px_rgba(245,158,11,.3)]"><button disabled={attackers.length>=4} onClick={()=>{const used=new Set(attackers.map(card=>card.slot_index));const slot=[1,2,3,4].find(n=>!used.has(n));if(slot)dropCard(selectedCard.id,"attacker",slot)}} className="rounded-lg border border-red-400 bg-red-950 px-4 py-2 text-[10px] font-black text-red-100 disabled:opacity-30">⚔️ JOGAR COMO ATACANTE</button><button disabled={mine("reinforcement").length>=4} onClick={()=>{const cards=mine("reinforcement");const used=new Set(cards.map(card=>card.slot_index));const slot=[1,2,3,4].find(n=>!used.has(n));if(slot)dropCard(selectedCard.id,"reinforcement",slot)}} className="rounded-lg border border-blue-400 bg-blue-950 px-4 py-2 text-[10px] font-black text-blue-100 disabled:opacity-30">🛡️ JOGAR COMO REFORÇO</button></motion.div>}</AnimatePresence>

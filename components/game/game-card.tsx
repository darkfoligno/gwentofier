...
export function GameCard({ card, interactive = false, className, isFaceDown = false }: { card?: GameCardType; interactive?: boolean; className?: string; isFaceDown?: boolean; playerMana?: number }) {
  ...
  return <><motion.article whileHover={interactive ? { y: -10, scale: 1.06, zIndex: 60 } : undefined} transition={{ type: "spring", stiffness: 320, damping: 24 }} className={cn("group relative aspect-[2/3] w-full rounded-[11px] border-2 p-[2px] shadow-xl", rarity[card.raridade], className)}>
    <div className="relative h-full overflow-hidden rounded-lg bg-gradient-to-b from-amber-700 via-yellow-950 to-amber-800 p-1.5 shadow-inner">
      <div className="relative flex h-full flex-col overflow-hidden rounded-md bg-[#FDFBF7] text-slate-900 shadow-[inset_0_0_18px_rgba(120,80,25,.16)]">
        <div className="relative z-20 ml-auto mr-2 mt-2 flex h-[12%] w-[76%] min-w-0 items-center justify-center rounded border border-amber-600 bg-white px-2 shadow-sm"><h3 title={card.nome} className="line-clamp-2 break-words text-center font-serif text-[clamp(7px,.8vw,11px)] font-black leading-tight">{card.nome}</h3></div>
        ...
        <div onClick={(event) => { event.stopPropagation(); if (card.image_url) setArtOpen(true) }} className="relative mx-2 mt-1 h-[45%] cursor-zoom-in overflow-hidden border-2 border-double border-amber-700 bg-stone-800">
        ...
        <div className="relative mx-2 min-h-0 flex-1 overflow-hidden border border-double border-amber-700/70 bg-white/70 p-1.5 text-[clamp(6px,.7vw,9px)] leading-snug"><strong>EFEITO: </strong>{highlightEffectText(card.efeito || "")}</div>
  ...
  </motion.article>{typeof document !== "undefined" && createPortal(<AnimatePresence>{artOpen && card.image_url && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={(event) => { event.stopPropagation(); setArtOpen(false) }} className="fixed inset-0 z-[500] flex cursor-zoom-out items-center justify-center bg-black/90 p-6 backdrop-blur-md"><img src={card.image_url} alt={card.nome} className="max-h-[88vh] max-w-[90vw] rounded-xl border-2 border-amber-400 object-contain shadow-[0_0_60px_rgba(245,158,11,.35)]" /></motion.div>}</AnimatePresence>, document.body)}</>
}

  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[180] overflow-y-auto bg-[radial-gradient(circle_at_top,rgba(127,29,29,.88),rgba(0,0,0,.98)_58%)] p-4 backdrop-blur-lg">
    <div className="mx-auto flex min-h-full w-full max-w-7xl flex-col justify-center py-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
        ...
        <section className="rounded-2xl border border-cyan-500/50 bg-stone-950/90 p-5"><h3 className="flex items-center gap-2 font-serif text-lg font-black uppercase text-cyan-200"><Zap/> Combos acionáveis</h3><p className="mt-1 text-xs text-stone-400">Mana dinâmica disponível: {mana} carta(s) na mão.</p><div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">{reactionCards.map(card=>{const effect=card.card_data?.effect_definition?.find(item=>item.trigger_type==="reaction"||item.is_reaction);const cost=Number(effect?.parameters?.mana_cost??card.card_data?.mana??0);const effectText=card.card_data?.efeito??effect?.description??"Reação defensiva desta carta.";return <article key={card.id} className="rounded-xl border border-cyan-400/50 bg-blue-950/50 p-3"><div className="mx-auto aspect-[2/3] w-24"><GameCard card={card.card_data??undefined}/></div><b className="mt-2 block text-sm text-cyan-100">{card.card_data?.nome}</b><p className="mt-1 min-h-12 text-[11px] leading-relaxed text-stone-300">{effectText}</p><button disabled={busy||reactionUsed||mana<cost} onClick={()=>void react(card.id)} className="mt-3 w-full animate-pulse rounded-lg border-2 border-yellow-200 bg-gradient-to-r from-amber-900 via-yellow-600 to-amber-900 px-2 py-3 text-[11px] font-black uppercase text-yellow-50 shadow-[0_0_20px_rgba(250,204,21,.65)] transition hover:brightness-125 disabled:animate-none disabled:opacity-35">⚡ Ativar efeito de reação · custo {cost}</button></article>})}</div>{!reactionCards.length&&<p className="mt-5 rounded-lg border border-stone-600 bg-black/60 p-5 text-center text-sm font-bold text-stone-300">Nenhuma reação válida disponível para este ataque</p>}<button disabled={busy} onClick={()=>void decline()} className="mt-5 w-full rounded-lg border-2 border-red-400 bg-gradient-to-r from-red-950 via-red-800 to-red-950 px-4 py-4 font-serif font-black uppercase text-red-50 disabled:opacity-50">{busy?"RESOLVENDO NO SERVIDOR…":"ACEITAR IMPACTO"}</button></section>
      </div>
    </div>
  </motion.div>
}

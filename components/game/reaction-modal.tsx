...
interface ReactionModalProps {
  ...
  onActivate: (cardId: string, effectOrder: number) => Promise<unknown>
...
export function ReactionModal({ attack, attackerCards, defenses, reactionCards, mana, reactionUsed, onActivate, onDecline }: ReactionModalProps) {
  ...
  const react=async(cardId:string,effectOrder:number)=>{if(busy||reactionUsed)return;setBusy(true);try{await onActivate(cardId,effectOrder)}finally{setBusy(false)}}
  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[180] overflow-y-auto bg-[radial-gradient(circle_at_top,rgba(88,28,135,.92),rgba(0,0,0,.98)_60%)] p-4 backdrop-blur-lg">
    <div className="mx-auto flex min-h-full w-full max-w-[1500px] flex-col justify-center py-4">
      <header className="mb-4 flex items-center justify-between rounded-2xl border-2 border-amber-300/80 bg-gradient-to-r from-purple-950 via-black to-amber-950 p-4 shadow-[0_0_60px_rgba(250,204,21,.28)]"><h2 className="flex items-center gap-2 font-serif text-xl font-black uppercase text-amber-100"><ShieldAlert/> Ataque recebido · escolha sua resposta</h2><div className="flex items-center gap-2 rounded-full border-2 border-amber-300 bg-purple-950 px-5 py-2 font-mono text-2xl font-black text-amber-100"><Timer size={20}/>{seconds}s</div></header>

...
type Contact = { request_id?: string; user_id: string; username: string; avatar_url: string | null; status: string; direction: "friend" | "received" | "sent"; last_seen?: string | null }
...
export function FriendsScreen() {
  ...
  const renderStatus = (lastSeen: string | undefined | null) => {
    if (!lastSeen) return <span className="ml-2 text-[10px] text-stone-500">Offline</span>
    if (isOnline(lastSeen)) return <span className="ml-2 flex items-center gap-1 text-[10px] text-emerald-400 font-bold"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Online agora</span>
    return <span className="ml-2 text-[10px] text-stone-400">Última vez online: {formatLastSeen(lastSeen)}</span>
  }

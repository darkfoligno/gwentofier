...
export function FriendsScreen() {
  ...
  <div className="grid gap-3 md:grid-cols-2">{contacts.map(contact => <article key={`${contact.direction}-${contact.user_id}`} className="flex items-center gap-3 rounded-lg border border-stone-700 bg-black/50 p-4">{contact.avatar_url ? <img src={secureImageUrl(contact.avatar_url)} alt="" className="h-12 w-12 rounded-full object-cover" /> : <div className="h-12 w-12 rounded-full bg-amber-900" />}<div className="min-w-0 flex-1"><b className="block truncate text-amber-100">{contact.username}</b><span className="text-xs text-stone-400">{contact.direction === "friend" ? "Amigo" : contact.direction === "received" ? "Convite recebido" : "Convite enviado"}</span></div>{contact.direction === "received" && contact.request_id && <div className="flex gap-1"><button aria-label="Aceitar" disabled={busy} onClick={() => void respond(contact.request_id!, true)} className="rounded bg-emerald-800 p-2"><Check size={16} /></button><button aria-label="Recusar" disabled={busy} onClick={() => void respond(contact.request_id!, false)} className="rounded bg-red-900 p-2"><X size={16} /></button></div>}</article>)}</div>{!contacts.length && !message && <p className="py-10 text-center text-stone-500">Nenhum contato ou convite no momento.</p>}
  
  {suggestions.length > 0 && <>
    <h2 className="mb-3 mt-10 font-serif text-xl font-bold text-amber-100">Jogadores da Arena</h2>
    <div className="grid gap-3 md:grid-cols-2">{suggestions.map(p => <article key={p.id} className="flex items-center gap-3 rounded-lg border border-stone-800 bg-stone-900/50 p-4">{p.avatar_url ? <img src={secureImageUrl(p.avatar_url)} alt="" className="h-12 w-12 rounded-full object-cover" /> : <div className="h-12 w-12 rounded-full bg-stone-800" />}<div className="min-w-0 flex-1"><b className="block truncate text-stone-300">{p.username}</b></div><button aria-label="Convidar" disabled={busy} onClick={() => { setUsername(p.username); setTimeout(() => send(), 100) }} className="rounded bg-stone-800 p-2 text-stone-300 hover:bg-emerald-900 hover:text-emerald-100"><UserPlus size={16} /></button></article>)}</div>
  </>}

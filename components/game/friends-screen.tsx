...
export function FriendsScreen() {
  ...
  const renderStatus = (lastSeen: string | undefined | null) => {
    if (!lastSeen) return <span className="text-[10px] text-stone-500">Offline</span>
    if (isOnline(lastSeen)) return <span className="flex items-center gap-1 text-[10px] text-emerald-400 font-bold"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> Online</span>
    const date = new Date(lastSeen)
    const pad = (n: number) => n.toString().padStart(2, "0")
    return <span className="text-[10px] text-stone-400">Offline ({pad(date.getHours())}:{pad(date.getMinutes())})</span>
  }

  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[178] overflow-y-auto bg-black/95 p-5 backdrop-blur-md">
    <motion.section initial={{scale:.92,y:25}} animate={{scale:1,y:0}} className="mx-auto w-full max-w-[1450px] rounded-2xl border-2 border-purple-300 bg-[radial-gradient(circle_at_top,#3b0764,#09090b_65%)] p-5 shadow-[0_0_70px_rgba(192,132,252,.45)]">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_300px_1fr]">
        <section className="min-w-0 rounded-xl border border-purple-400/30 bg-black/45 p-4"><h3 className="mb-3 text-xs font-black uppercase text-purple-200">Cartas elegíveis · arraste ou selecione</h3><div className="flex min-h-56 gap-4 overflow-x-auto pb-3">{cards.map(card=>{const badge=zoneBadge[card.zone];return <button draggable key={card.id} onDragStart={event=>{event.dataTransfer.effectAllowed="move";event.dataTransfer.setData("text/effect-card-id",card.id)}} onClick={()=>setSelectedId(card.id)} className={`relative w-36 shrink-0 rounded-xl border p-2 pt-9 transition hover:-translate-y-2 ${selected?.id===card.id?"border-yellow-200 bg-yellow-900/25 shadow-[0_0_22px_rgba(250,204,21,.45)]":"border-purple-300/40 bg-purple-950/35"}`}><span className={`absolute inset-x-1 top-1 rounded border px-1 py-1 text-[7px] font-black uppercase tracking-tight ${badge?.style??"border-stone-500 bg-stone-900 text-stone-200"}`}>{badge?.label??`📍 ZONA: ${card.zone}`}</span><div className="aspect-[2/3] w-full"><GameCard card={card.card_data??undefined} enableZoom={false}/></div><b className="mt-2 block text-[10px] text-purple-100">{card.card_data?.nome}</b><span className="text-[9px] font-black text-cyan-200">⚡ Custo {costOf(card)}</span></button>})}{!cards.length&&<p className="m-auto text-sm text-stone-400">Nenhum efeito de turno disponível nas zonas autorizadas.</p>}</div></section>

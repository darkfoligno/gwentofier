...
export function StoreScreen() {
  ...
  const daily = async () => { setBusy("daily"); setError(null); const user = (await supabase.auth.getUser()).data.user; if (!user) return; const { data, error: rpcError } = await supabase.rpc("claim_daily_login_reward", { p_user_id: user.id }); setBusy(null); if (rpcError) { setError(rpcError.message); return } await refresh() }
  ...
  return <main className="relative min-h-screen overflow-hidden bg-[url('/yang-69TcSUVhbmY-unsplash.jpg')] bg-cover bg-fixed bg-center p-6 text-stone-100"><div className="absolute inset-0 bg-black/80 backdrop-blur-[2px]" /><div className="relative mx-auto max-w-7xl">
    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">{packs.map((pack, index) => <motion.article key={pack.id} whileHover={{ scale: 1.05 }} transition={{ duration: 0.3 }} className="relative overflow-hidden rounded-xl border-2 border-amber-600/40 bg-zinc-950 p-5 shadow-2xl hover:border-amber-400"><div className="mb-5 flex h-44 items-center justify-center rounded-lg border border-amber-700/50 bg-[radial-gradient(circle,rgba(120,53,15,0.4),rgba(24,24,27,1)_65%)]">{index === packs.length - 1 ? <Sparkles className="animate-pulse text-amber-300" size={70} /> : <div className="font-serif text-6xl text-amber-300/80">🕮</div>}</div><h2 className="font-serif text-xl font-black text-amber-100">{pack.name}</h2><p className="mt-2 min-h-20 text-sm leading-relaxed text-zinc-400">{pack.description ?? ""}</p><div className="mt-5 flex items-center justify-between"><span className="flex items-center gap-1 text-lg font-black text-amber-300"><Coins size={18} />{pack.price_coins}</span><button disabled={Boolean(busy)} onClick={() => void purchase(pack)} className="rounded border border-amber-400 bg-amber-700 px-4 py-2 text-[10px] font-black text-amber-50 shadow-[0_0_10px_rgba(245,158,11,0.2)] disabled:opacity-40">{busy === pack.id ? "ABRINDO..." : "COMPRAR E ABRIR"}</button></div></motion.article>)}</div>

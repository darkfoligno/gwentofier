-- Migration 202608030292_fix_match_deck_cards_schema_align.sql
-- Alinha a inicialização do deck do bot de treino ao esquema real sem colunas inexistentes
...
BEGIN
    ...
    -- Popular match_cards para o player e para o bot (utilizando row_number() como zone_position)
    IF p_deck_id = 'SYSTEM_GENERATED' OR p_deck_id IS NULL THEN
        ...
        SELECT v_match_id, v_user_id, v_user_id, mdc.id, mdc.source_card_id, 'deck', row_number() over (order by mdc.id)::integer, false, mdc.base_power, mdc.base_max_life, mdc.base_power, mdc.base_power, mdc.base_max_life, mdc.base_max_life
        FROM public.match_deck_cards mdc WHERE mdc.match_deck_id = v_player_match_deck_id;
    END IF;

    INSERT INTO public.match_cards(match_id, owner_user_id, controller_user_id, match_deck_card_id, source_card_id, zone, zone_position, is_face_up, base_power, base_max_life, current_power, maximum_power, current_life, maximum_life)
    SELECT v_match_id, v_bot_id, v_bot_id, mdc.id, mdc.source_card_id, 'deck', row_number() over (order by mdc.id)::integer, false, mdc.base_power, mdc.base_max_life, mdc.base_power, mdc.base_power, mdc.base_max_life, mdc.base_max_life
